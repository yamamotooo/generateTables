...
### SummaryInfo（集計情報）※ fieldType が 集計タイプ の場合のみ出力

集計タイプのフィールドでは、`dataType` 列と `Calculation` 列を以下のように流用します。

| config.xml 属性 | 流用元 | 内容 | 書式 |
|---|---|---|---|
| `SummaryInfo` の種別 | `Field dataType`（K列） | 繰り返し区分と集計操作の組み合わせ | `繰り返し区分.操作` |
| `SummaryField.Field` の参照 | `Calculation`（Q列） | 集計対象フィールドの ID と名前 | `id.フィールド名` |

**K列（種別）の許可値**

| Excel 入力値 | summarizeRepetition | operation |
|---|---|---|
| `Together.Total` | `Together` | `Total`（合計） |
| `Together.Average` | `Together` | `Average`（平均） |
| `Together.Count` | `Together` | `Count`（個数） |
| `Together.List` | `Together` | `List`（一覧） |
| `Individually.Total` | `Individually` | `Total`（合計・個別） |
| `Individually.Average` | `Individually` | `Average`（平均・個別） |
| `Individually.Count` | `Individually` | `Count`（個数・個別） |
| `Individually.List` | `Individually` | `List`（一覧・個別） |

**Q列（SummaryField 参照）の書式**

```
フィールドID.フィールド名
```

例：`1.Hoge`（id=1、フィールド名 Hoge を集計対象にする）

**出力される XML 例**

```xml
<Field id="5" name="s_合計" fieldType="Summary" dataType="Number">
  <SummaryInfo restartForEachSortedGroup="False" summarizeRepetition="Together" operation="Total">
    <SummaryField>
      <Field id="1" name="Hoge"></Field>
    </SummaryField>
  </SummaryInfo>
  <Comment>合計フィールド</Comment>
  ...
</Field>
```

> **注意：** 集計タイプの `dataType` は常に `Number` で出力されます。
...
| `AutoEnter overwriteExistingValue` | 既存値を上書き | `False` | `True` / `False` |
| `AutoEnter allowEditing` | 編集を許可 | `True` | `True` / `False` |
...
| `Unique value` | 値の一意性を検証 | `False` | `True` / `False` |
| `NotEmpty value` | 空を許可しない | `False` | `True` / `False` |
| `MaxDataLength value` | 最大文字数（空の場合は制限なし） | 空 | 数値 |
| `Existing value` | 既存値との重複を検証 | `False` | `True` / `False` |
